...
describe("filtrarLinesPorColaborador — filtro por colaborador (matrícula escolhida na lista da tela)", () => {
  it("filtra só as linhas da matrícula escolhida — não precisa consultar Colaboradores, matrícula já vem na linha calculada", async () => {
  ...
  it("não faz match parcial de matrícula — precisa ser a matrícula inteira", async () => {
    const linhas = [linha({ matricula: 90103362 }), linha({ matricula: 9010336 })];
    const resultado = await filtrarLinesPorColaborador(linhas, { colaborador: "90103362" });
    expect(resultado).toHaveLength(1);
    expect(resultado[0].matricula).toBe(90103362);
  });

  it("combina com o filtro de regime — mesma matrícula em dois regimes, só bate o filtrado", async () => {
    const linhas = [linha({ matricula: 1, fpas: 515 }), linha({ matricula: 1, fpas: 655 })];
    const resultado = await filtrarLinesPorColaborador(linhas, { colaborador: "1", fpas: 655 });
    expect(resultado).toHaveLength(1);
