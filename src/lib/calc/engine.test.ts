...
describe("calculateLine — trilha de encargos (proventos/descontos)", () => {
  ...
  it("Gross Up customizado do Tomador multiplica a fatura (mesma fórmula pra qualquer regime)", async () => {
  ...
  it("usa INSS 655 e encargo sobre provisão baseado só na Prov. 13º quando o tomador é FPAS 655 (Temporario)", async () => {
    ...
    // NF não depende do regime — mesma fórmula multiplicativa pra Terceiro e Temporário.
    expect(l.tomadorGrossUp).toBeCloseTo(0.1325, 6);
    expect(l.nf).toBeCloseTo(l.fatura * 1.1325, 6);
  });

  it("Temporário (655): Gross Up 0 desliga — NF = fatura, igual ao Terceiro", async () => {
    await upsertTomador({ ...TOMADOR_TEMPORARIO, grossUp: 0 });
    await upsertColaborador({
      matricula: 1,
      dados: { cod_epr: 1, nome: "FULANO", situacao: "Trabalhando", cod_servico: TOMADOR_TEMPORARIO.codigo },
    });
  ...
  it("Tomador código 14 (ITAU) não é caso especial — usa a mesma fórmula multiplicativa de qualquer outro Tomador", async () => {
    ...
    await upsertColaborador({
      matricula: 1,
      dados: { cod_epr: 1, nome: "FULANO", situacao: "Trabalhando", cod_servico: 14 },
    });
    const mov: Movimento = {
      id: "1",
      codigo: 8781,
